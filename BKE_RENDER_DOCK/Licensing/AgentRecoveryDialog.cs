...
internal sealed class AgentRecoveryDialog : Form
{
    ...
    private AgentRecoveryDialog()
    {
        ...
        download.Click += (_, _) => { OpenRecoveryPage(); Close(); };
        CancelButton = close;
    ...
    private static void OpenRecoveryPage()
    {
        try { Process.Start(new ProcessStartInfo { FileName = RecoveryUrl, UseShellExecute = true }); }
        catch (Exception) { }
    }
}
